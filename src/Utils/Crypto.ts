...
export async function pbkdf2(params: pbkdf2params){
    const subtleAvailable = !!(global.crypto?.subtle?.importKey) && !!(global.crypto?.subtle?.deriveBits)
    ...
    if(subtleAvailable) return await pbkdf2Subtle(params)
        ...
        return await pbkdf2Async(sha256, params.password, params.salt, {c: params.iterations, dkLen: params.dkLen})
    }
}
