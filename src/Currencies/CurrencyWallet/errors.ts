...
export class NotEnoughFundsError<CI extends CurrencyInfo> extends Error {
    public readonly missingFunds?: CurrencyAmount<CI>

    constructor(missingFunds?: CurrencyAmount<CI>) {
        if (missingFunds) {
            super(
                `Wallet does not have enough funds. ${missingFunds.baseAmount} ${missingFunds.baseSymbol} needed.`,
            )
        } else {
